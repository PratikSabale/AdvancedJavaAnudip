...
public class Hotel {
	...
	@Pattern(regexp = "^(?=.*[A-Z])(?=.*[a-z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{8,20}$", message = "Password must be 8-20 characters long, include at least one uppercase letter, one lowercase letter, one number, and one special character.")
	// @Pattern
	// ensures
	// secure
	// password
	...
	public String toString() {
		return "Hotel [name=" + name + ", hoteltype=" + hoteltype + ", quality=" + quality + ", password=" + password
				+ ", id=" + id + "]";
	}
